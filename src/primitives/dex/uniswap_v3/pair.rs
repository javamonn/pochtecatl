use super::{
    super::{DexPair, DexPairInput, IndexedTrade},
...
use crate::{
    ...
    trade_controller::TradeRequestOp,
};
...
impl DexPairInput<UniswapV3Pair> for UniswapV3PairInput {
    ...
    fn decode(&self, result: Vec<multicall3::Result>) -> Result<UniswapV3Pair> {
        ...
        if factory_returns._0 != *config::UNISWAP_V3_FACTORY_ADDRESS {
            Err(eyre!("pair does not belong to UniswapV3 factory"))
        } else if token0_returns._0 != *config::WETH_ADDRESS
            && token1_returns._0 != *config::WETH_ADDRESS
        {
...
impl UniswapV3Pair {
        ...
        P: Provider<T, Ethereum> + 'static,
    {
        let multicall_tx_request = multicall_tx_request(vec![
            ...
            multicall3::Call3 {
                target: *config::UNISWAP_V3_QUOTER_V2_ADDRESS,
                allowFailure: false,
...
impl DexPair<UniswapV3IndexedTrade> for UniswapV3Pair {
        ...
        P: Provider<T, alloy::network::Ethereum> + 'static,
    {
        let (amount0, amount1, sqrt_price_x96, liquidity) = match op {
            ...
            TradeRequestOp::Close(IndexedTrade::UniswapV3(open_trade)) => {
                let open_trade_token_amount_out = if self.token0 == *config::WETH_ADDRESS {
        ...
        P: Provider<T, Ethereum> + 'static,
    {
        match op {
            TradeRequestOp::Open => {
                ...
                let tx_request = TransactionRequest::default()
                    .with_from(rpc_provider.signer_address().clone())
                    .with_to(Into::<TxKind>::into(*config::UNISWAP_V3_ROUTER_02_ADDRESS))
                    .with_input(
...
mod tests {
    use crate::{
        config,
        primitives::dex::{DexPair, UniswapV3IndexedTrade, UniswapV3Pair},
